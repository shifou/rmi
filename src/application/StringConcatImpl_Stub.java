...
/**
 * StringConcatImpl_Stub is the stub for StringConcatImpl application of the
 * interface. So when concat is invoked, the stub connects with the server where
 * StringConcarImpl is hosted, sends the server a message to invoke concat with
 * arguments a and b, reads the message reply from the server, determines
	...
	public String concat(String a, String b) throws Remote440Exception {
		...
		try {
			...
			Message message = new Message(msgType.INVOKE, args, new String(
					"concat"), new String(this.identifier));
